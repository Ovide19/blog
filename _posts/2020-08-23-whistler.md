...
This post will cover a script allowing to plot the spectrogram of Very Low Frequency (VLF) magnetic data, while simultaneously listening to the associated audio file.
...
If we simply wanted to convert this time series into an audio track, we could use the script below:

```python
from scipy.io import loadmat
from scipy.io.wavfile import write

matfilename = 'PA120101021000_021.mat' #Name of the mat file downloaded from WALDO
filename = matfilename.split('.')[0]+'.wav' #Name of the target wav file
fs = 100000 #Sampling rate of the VLF data
x = loadmat(matfilename) 
data=x['data'][:,0][:] 
write(filename, fs, data)
```

Upon importing SciPy's I/O `loadmat` method, the aforementioned .mat file can be loaded into dictionary `x` using `x = loadmat(matfilename)`.
The `data` stored in `x` can then be written to a .wav file using `write` method from SciPy's I/O `wavfile` module: `write(filename, fs, data)`.

If you open the .wav file using your favourite media player, you should start hearing popping and cracking sounds, some of which are associated with **sferics**.
...
If all goes well, you should start hearing popping and cracking sounds while the data's spectrogram shows up:

![Spectrogram](/blog/assets/images/whistler.png)

So what's up with the whistlers mentioned in this post's title? Well, if you have the patience to wait for 50 seconds, you should start hearing an eerie whistling sound, while the vertical cursor reaches the dark blue banana-shaped feature to the right of the spectrogram. This wave is produced by lightning strikes and due to interaction with the ionosphere, it travels along the Earth's geomagnetic field lines, from one hemisphere to the other. The banana shape is due to dispersion: through plasma environments of the ionosphere and the magnetosphere, the low frequency component of the signal travels at lower velocity than the high frequency component.
